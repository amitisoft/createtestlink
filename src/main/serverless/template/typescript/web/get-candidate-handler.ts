...
export class GetCandidateHandler {
    ...
    // static findByEmail(httpContext:HttpContextImpl,injector:Injector) : void {

    //       let pathParameters = httpContext.getPathParameters();
    //      console.log(JSON.stringify(pathParameters));
    //     let data = httpContext.getRequestBody();
    //      injector.get(CandidateFacade).findByEmail(data)
    //          .subscribe(result => {
    //       injector.get(CandidateFacade).findByEmail(result) //findByCandidateId(id)
    //           .subscribe(result1 => {
    //              httpContext.ok(200, result);
    //      },  err => {
    //              httpContext.fail(err, 500);
